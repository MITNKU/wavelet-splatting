...
class SimpleTrainer2d:
    """Trains random 2d wipes to fit an image."""
    def __init__(
        self,
        image_path: Path,
        num_points: int = 2000,
        model_name:str = "WIPESImage_Cholesky",
        ...
        if model_name == "WIPESImage_Cholesky":
            from wipesimage_cholesky import WIPESImage_Cholesky
            self.wipes_model = WIPESImage_Cholesky(loss_type="L2", opt_type="adan", num_points=self.num_points, H=self.H, W=self.W, BLOCK_H=BLOCK_H, BLOCK_W=BLOCK_W, 
                device=self.device, lr=args.lr, quantize=False, image_path=image_path).to(self.device)

        elif model_name == "WIPESImage_RS":
            from wipesimage_rs import WIPESImage_RS
            self.wipes_model = WIPESImage_RS(loss_type="L2", opt_type="adam", num_points=self.num_points, H=self.H, W=self.W, BLOCK_H=BLOCK_H, BLOCK_W=BLOCK_W, 
                device=self.device, lr=args.lr, quantize=False, normf_lr=0.01).to(self.device) 
        ...
        if model_path is not None:
            ...
            model_dict = self.wipes_model.state_dict()
            pretrained_dict = {k: v for k, v in checkpoint.items() if k in model_dict}
            model_dict.update(pretrained_dict)
            self.wipes_model.load_state_dict(model_dict)

    def train(self):     
        psnr_list, iter_list = [], []
        progress_bar = tqdm(range(1, self.iterations+1), desc="Training progress")
        best_psnr = 0
        self.wipes_model.train()
        start_time = time.time()
        for iter in range(1, self.iterations+1):
            loss, psnr = self.wipes_model.train_iter(self.gt_image, iter)
        ...
        torch.save(self.wipes_model.state_dict(), self.log_dir / "wipes_model.pth.tar")
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--model_name", type=str, default="WIPESImage_Cholesky", help="model selection: WIPESImage_Cholesky, WIPESImage_RS"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=70000,
        help="2D WIPES points (default: %(default)s)",
    )
...
def main(argv):
    ...
    for i in range(start, start+image_length):
        if args.data_name == "kodak":
            ...
            image_path = Path(args.dataset) / f'exp{i+1:02}.png'
